...
lena_smooth = myFilter(lena,kernel);
lena_smooth = (lena_smooth/max(lena_smooth(:)));
subplot(2,3,2);
imshow(lena_smooth);
title("Smooth Image");
hold on;

lena_gary = rgb2gray(lena_smooth);
subplot(2,3,3);
imshow(lena_gary);
title("Gary Image");
hold on;
...
edge_result = Hysteresis_Thresholding(lena_nonMax,0.3,0.07);
subplot(2,3,6);
imshow(edge_result);
